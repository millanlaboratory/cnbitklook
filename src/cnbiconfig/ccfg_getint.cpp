...
void usage(void) { 
	printf("Usage: ccfg_getint [OPTION]...\n\n");
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		std::cerr << "[ccfg_getint] Exception: " << e << std::endl;
		return -1;
	}

	try {
		val = config->RootEx()->QuickIntEx(parampath);
	} catch(XMLException e){
		val=0;
		std::cerr << "[ccfg_getint] Exception: " << e << std::endl;
		std::cerr << "[ccfg_getint] Parameter path is probably wrong" << std::endl;
		return -2;
	}
