...
void usage(void) { 
	printf("Usage: ccfg_getbool [OPTION]...\n\n");
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		std::cerr << "[ccfg_getbool] Exception: " << e << std::endl;
		return -1;
	}

	try {
		val = config->RootEx()->QuickBoolEx(parampath);
	} catch(XMLException e){
		val=0;
		std::cerr << "[ccfg_getbool] Exception: " << e << std::endl;
		std::cerr << "[ccfg_getbool] Parameter path is probably wrong" << std::endl;
		return -2;
	}
