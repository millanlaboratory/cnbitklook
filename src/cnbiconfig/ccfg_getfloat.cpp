...
void usage(void) { 
	printf("Usage: ccfg_getfloat [OPTION]...\n\n");
...
int main(int argc, char *argv[]) {
	int opt;
	std::string xmlfilepath, parampath, rootname = "cnbiconfig";
	float val = 0.0f;
	...
	try {
		...
		std::cerr << "[ccfg_getfloat] Exception: " << e << std::endl;
		return -1;
	}

	try {
		val = config->RootEx()->QuickFloatEx(parampath);
	} catch(XMLException e){
		val=0.0f;
		std::cerr << "[ccfg_getfloat] Exception: " << e << std::endl;
		std::cerr << "[ccfg_getfloat] Parameter path is probably wrong" << std::endl;
		return -2;
	}
